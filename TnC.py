...
class TravelerSearch:
    def search(self, from_city, to_city, departure_pincode, departure_date=None):
        ...
        try:
            ...
            if departure_date:
                results = results[results['departure_date'] == str(departure_date)]
...
def render_search_page():
    ...
    st.markdown('<div class="custom-card">', unsafe_allow_html=True)
    st.markdown("<h3>Find a Traveler</h3><p>Enter a route and date to find registered travelers who can carry your package.</p>", unsafe_allow_html=True)
    
    with st.form("search_form"):
        col1, col2 = st.columns(2)
        with col1: 
            search_from = st.text_input("From City", placeholder="e.g., Pune")
            search_pincode = st.text_input("Departure Pincode", placeholder="e.g., 411033", max_chars=6)
        with col2: 
            search_to = st.text_input("To City", placeholder="e.g., Delhi")
            search_departure_date = st.date_input("Departure Date", min_value=date.today(), help="Leave blank to search all dates")
        
        # Add checkbox to make date search optional
        use_date_filter = st.checkbox("Filter by specific departure date", value=True, help="Uncheck to search all departure dates")
    ...
    if search_btn:
        ...
        if errors:
            ...
            with st.spinner("Searching for travelers..."):
                # Pass departure date only if the checkbox is checked
                departure_date_filter = search_departure_date if use_date_filter else None
                results = searcher.search(search_from, search_to, search_pincode, departure_date_filter)
            
            # Update the search results header to show the search criteria
            search_criteria = f"Route: {search_from} → {search_to} | Pincode: {search_pincode}"
            if use_date_filter:
                search_criteria += f" | Date: {search_departure_date}"
            
            st.markdown(f"### Search Results ({len(results)})")
            st.markdown(f"*{search_criteria}*")
            
            if results.empty:
                if use_date_filter:
                    st.warning("No travelers found for this route and date. Be the first to register for it!")
                else:
