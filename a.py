...
class To:
    ...
    def search_travelers(self, from1, to, departure_pincode=None, departure_date=None):
        ...
        try:
            ...
            df['from1'] = df['from1'].fillna('').astype(str)
            df['to'] = df['to'].fillna('').astype(str)
            df['departure_pincode'] = df['departure_pincode'].fillna('').astype(str)

            # Filter by route (case-insensitive)
            output_df = df[
                (df['from1'].str.lower().str.strip() == str(from1).lower().strip()) &
                (df['to'].str.lower().str.strip() == str(to).lower().strip())
            ]

            if departure_pincode:
                output_df = output_df[
                    output_df['departure_pincode'].str.lower().str.strip() == str(departure_pincode).lower().strip()
                ]
...
def main():
    ...
    with tab1:
        ...
        with st.form("traveler_form"):
            ...
            with col2:
                ...
                departure_pincode = st.text_input("Departure Pincode *", help="City you're living in. Pincode.")
    ...
    with tab2:
        ...
        with st.form("search_form"):
            ...
            with col3:
                departure_pincode = st.text_input("Departure Pincode", help="Enter your departure pincode.")       
            with col4:
                        ...
                        for index, row in results.iterrows():
                            with st.expander(f"✈️ {row['name']} {row['surname']} - {row['from1']} → {row['to']}"):
                                ...
                                with col2:
                                    ...
                                    st.write(f"**Departure Pincode:** {row['departure_pincode']}")
